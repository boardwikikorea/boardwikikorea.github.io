import{w as r,s as i}from"./index-DZSsd85T.js";async function o(n,e=[]){return r(async()=>{const{data:t,error:a}=await i.rpc("apply_event",{p_event_id:n,p_options:e.map(s=>({option_id:s.optionId,music_text:s.musicText??null,memo_text:s.memoText??null}))});if(a)throw a;return{registrationId:t}})}async function c(){return r(async()=>{const{data:n,error:e}=await i.from("view_my_event_registrations").select(`
        registration_id,
        registration_state,
        registered_at,
        cancel_requested,
