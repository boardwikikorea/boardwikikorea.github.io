...
`;async function w(){return l(async()=>{const{data:r,error:t}=await i.from("spot_info").select(g);if(t)throw t;return(r??[]).map(c).sort((n,e)=>{const o=m(e.updatedAt??e.createdAt),s=m(n.updatedAt??n.createdAt),u=o-s;return u!==0?u:n.name.localeCompare(e.name)})})}async function S(r){return l(async()=>{const{data:t,error:a}=await i.from("spot_info").select(g).eq("id",r).maybeSingle();if(a)throw a;return t?c(t):null})}async function k(r){return l(async()=>{var e;const{data:t,error:a}=await i.from("location").insert({name:r.name.trim(),address:r.address.trim(),address_detail:r.addressDetail.trim(),google_map_url:((e=r.googleMapUrl)==null?void 0:e.trim())||null,description:r.description.trim(),thumbnail_url:r.thumbnailUrl,status:r.status,type:"spot",lat:r.lat,lon:r.lon}).select("id").single();if(a)throw a;const{error:n}=await i.from("spot_info").upsert({id:t.id,floor:r.floor,light:r.light,parking:r.parking,toilet:r.toilet,roof:r.roof},{onConflict:"id"});if(n)throw n;return t.id})}async function q(r,t){return l(async()=>{var e;const{error:a}=await i.from("location").update({name:t.name.trim(),address:t.address.trim(),address_detail:t.addressDetail.trim(),google_map_url:((e=t.googleMapUrl)==null?void 0:e.trim())||null,description:t.description.trim(),thumbnail_url:t.thumbnailUrl,status:t.status,type:"spot",lat:t.lat,lon:t.lon}).eq("id",r);if(a)throw a;const{error:n}=await i.from("spot_info").update({floor:t.floor,light:t.light,parking:t.parking,toilet:t.toilet,roof:t.roof}).eq("id",r);if(n)throw n})}async function E(r,t){return l(async()=>{const{data:a,error:n}=await i.from("location").update({status:t}).eq("id",r).select("id,status");if(n)throw n;const e=(a??[])[0]??null;if(!e)throw new Error("Spot status was not updated. Check location row access or RLS policy.");return e.status})}async function P(r){return l(async()=>{const{data:t,error:a}=await i.from("location").select("id,thumbnail_url").eq("id",r).maybeSingle();if(a)throw a;const{data:n,error:e}=await i.from("image").select("id,related_id,image_url,thumbnail_url").eq("related_type","location").eq("related_id",r).order("created_at",{ascending:!1}).order("id",{ascending:!1});if(e)throw e;return(n??[]).map(o=>y(o,(t==null?void 0:t.thumbnail_url)??null))})}async function A(r,t){return l(async()=>{if(!t){const{error:o}=await i.from("location").update({thumbnail_url:null}).eq("id",r);if(o)throw o;return}const{data:a,error:n}=await i.from("image").select("id,related_id,thumbnail_url").eq("id",t).eq("related_type","location").eq("related_id",r).single();if(n)throw n;const{error:e}=await i.from("location").update({thumbnail_url:a.thumbnail_url}).eq("id",r);if(e)throw e})}async function U(r){return l(async()=>{const{error:t}=await i.from("image").delete().eq("id",r);if(t)throw t})}export{S as a,P as b,k as c,U as d,q as e,w as f,A as s,E as u};
