...
`,L=`
  ...
  created_at,
  updated_at
`;async function m(e,t){const r=k(t),{error:i}=await n.from("crew_social_link").delete().eq("crew_id",e);if(i)throw i;if(r.length===0)return;const{error:s}=await n.from("crew_social_link").insert(r.map((_,a)=>({crew_id:e,type:_.type,url:_.url,sort_order:a})));if(s)throw s}async function I(){return f("crew:active:list",6e4,()=>o(async()=>{const{data:e,error:t}=await n.from("crew").select(S).eq("is_active",!0);if(t)throw t;return(e??[]).map(d).sort((r,i)=>{const s=l(i.updatedAt??i.createdAt),_=l(r.updatedAt??r.createdAt),a=s-_;return a!==0?a:r.name.localeCompare(i.name)})}))}async function W(){return o(async()=>{const{data:e,error:t}=await n.from("crew").select(L);if(t)throw t;return(e??[]).map(d).sort((r,i)=>{const s=l(i.updatedAt??i.createdAt),_=l(r.updatedAt??r.createdAt),a=s-_;return a!==0?a:r.name.localeCompare(i.name)})})}async function q(e){return o(async()=>{const{data:t,error:r}=await n.from("crew").select(T).eq("id",e).maybeSingle();if(r)throw r;return t?d(t):null})}async function P(e){return f(`crew:active:id:${e}`,6e4,()=>o(async()=>{const{data:t,error:r}=await n.from("crew").select(v).eq("id",e).eq("is_active",!0).maybeSingle();if(r)throw r;return t?d(t):null}))}async function R(e){return o(async()=>{var i;const{data:t,error:r}=await n.from("crew").insert({name:e.name.trim(),activity_area:((i=e.activityArea)==null?void 0:i.trim())||null,country_code:e.countryCode,primary_spot_id:e.primarySpotId,chat_link:c(e.chatLink),content:c(e.content),logo_url:e.logoUrl,fee_use:e.feeUse,fee_type:e.feeUse?e.feeType:null,fee_text:e.feeUse?c(e.feeText):null,is_active:e.isActive}).select("id").single();if(r)throw r;return await m(t.id,e.socialLinks),t.id})}async function b(e,t){return o(async()=>{var i;const{error:r}=await n.from("crew").update({name:t.name.trim(),activity_area:((i=t.activityArea)==null?void 0:i.trim())||null,country_code:t.countryCode,primary_spot_id:t.primarySpotId,chat_link:c(t.chatLink),content:c(t.content),logo_url:t.logoUrl,fee_use:t.feeUse,fee_type:t.feeUse?t.feeType:null,fee_text:t.feeUse?c(t.feeText):null,is_active:t.isActive}).eq("id",e);if(r)throw r;await m(e,t.socialLinks)})}async function B(e,t){return o(async()=>{const{error:r}=await n.from("crew").update({is_active:t}).eq("id",e);if(r)throw r})}export{C,P as a,W as b,w as c,q as d,R as e,I as f,b as g,B as u};
